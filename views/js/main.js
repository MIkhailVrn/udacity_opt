...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
	...
	// get all neccessary data outside the loop   
	var rndmPizzaContainer = document.querySelectorAll(".randomPizzaContainer");
	var length = document.querySelectorAll(".randomPizzaContainer").length;
	var windowWidth = document.querySelector("#randomPizzas").offsetWidth;
	var oldWIdth = rndmPizzaContainer[0].offsetWidth;
	var dx = determineDx(size, windowWidth, oldWIdth);
    var newwidth = (oldWIdth + dx) + 'px';
    for (var i = 0; i < length; i++) {
      rndmPizzaContainer[i].style.width = newwidth;
    }
  }
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(( scrl / 1250) + (i % 5));
